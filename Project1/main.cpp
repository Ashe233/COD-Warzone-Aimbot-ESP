...
PVOID GetRemotePEBAddress(DWORD processId) {
...
int main()
{
	...
	while (!hwnd)
	{
		hwnd = FindWindowW((L"CoD Focused Mode Window"),NULL);
		Sleep(1000);
		std::cout << "[+]Finding COD...\n";
	}
	while (!oPID) // get the process id
	{
		oPID = GetPID("cod.exe");
		printf("[+] Status COD:Found\n");
		Sleep(500);
	}

	while (!oBaseAddress) // request the module base from driver
	{
		oBaseAddress = GetModuleBaseAddress(oPID, "cod.exe");
	...
	screenWeight = therefofgame.Width;
	screenHeight = therefofgame.Height;
	Pebadd = (uint64_t)GetRemotePEBAddress(oPID);
