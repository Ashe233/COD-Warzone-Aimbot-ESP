...
#pragma pack(push, 1)
	...
	public:
		...
		__forceinline Vec3 operator*(float Scale) const {
		...
		__forceinline Vec3 operator+(float A) const {
		...
		__forceinline float operator|(const Vec3& V) const {
		...
		__forceinline bool operator==(const Vec3& src) const {
		...
		__forceinline Vec3 Rotate(float angle)
		{
			return Vec3(X * cos(-angle) - Z * sin(-angle), Y, X * sin(-angle) + Z * cos(-angle));
		}
		...
		__forceinline float DistTo(Vec3 targetTo) const {
			return (targetTo - *this).Size();
		}

		__forceinline Vec3 toRotator(Vec3 targetTo)
		{
			Vec3 Normalized = (targetTo - *this);
	...
	public:
		...
		Vec3 GetCameraVec()
		{
			...
			return Vec3(-(A * m41 + D * m42 + G * m43) / Z, -(B * m41 + E * m42 + H * m43) / Z, -(C * m41 + F * m42 + K * m43) / Z);
		}

		bool WorldToScreen(Vec3 worldPos, Vector2* OutPos, Vector2 WindowSize, bool ignoreret = false)
		{
		...
		Vector2 WorldToScreen(Vec3 worldPos)
		{
			float screenX = (r[0].m128_f32[0] * worldPos.X) + (r[1].m128_f32[0] * worldPos.Y) + (r[2].m128_f32[0] * worldPos.Z) + r[3].m128_f32[0];
			float screenY = (r[0].m128_f32[1] * worldPos.X) + (r[1].m128_f32[1] * worldPos.Y) + (r[2].m128_f32[1] * worldPos.Z) + r[3].m128_f32[1];
			float screenW = (r[0].m128_f32[3] * worldPos.X) + (r[1].m128_f32[3] * worldPos.Y) + (r[2].m128_f32[3] * worldPos.Z) + r[3].m128_f32[3];
